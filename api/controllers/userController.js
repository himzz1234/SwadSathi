...
const register = async(req, res)=>{
    ...
    try {
      ...
      success = true;
      res.json({ success, message: 'Account created successfully!', token: authtoken, });
    } catch (error) {
...
const login = async(req, res)=>{
    ...
    try {
      ...
      if(user && passwordCompare){
        const authtoken = generateToken(user._id)
        success = true
        res.json({ success, message: 'Logged In successfully!',token: authtoken, id: user._id, name: user.name, email: user.email, savedCanteens: user.savedCanteens });
      }
      else{
        success = false
